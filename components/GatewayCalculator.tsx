    ...
    const renderStep1 = () => (
        <div className="p-4 md:p-8">
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h2 className="text-2xl font-bold">Step 1: Define Camera Streams</h2>
                    <p className="text-on-surface-variant mt-1">Add cameras and specify their recording and streaming settings.</p>
                </div>
                 <div className="flex items-center gap-2">
                    <button onClick={handleImportFromProject} className="px-3 py-2 text-sm bg-surface hover:bg-white/10 border border-white/20 rounded-md flex items-center gap-2" title="Import cameras marked for gateway use from the current project">
                        <CameraIcon className="w-4 h-4" /> <span>Import</span>
                    </button>
                    <button onClick={handleAddCamera} className="px-3 py-2 text-sm bg-primary-600 text-white rounded-md flex items-center gap-2">
                        <AddIcon className="w-4 h-4" /> <span>Add Camera</span>
                    </button>
                    <button type="button" onClick={() => handleNavigateToStep(2)} className="px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700 transition-colors flex items-center gap-2 text-sm">
                        <span>Next</span> <ChevronsRightIcon className="w-4 h-4" />
                    </button>
                </div>
            </div>
    ...
    return (
        <div ref={viewRef} className="w-full h-full flex flex-col">
            <header className="p-4 md:p-6 border-b border-white/10 flex-shrink-0 flex justify-between items-center bg-surface">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={resetState} className="text-sm px-3 py-1.5 rounded-md hover:bg-white/10">Reset</button>
                    <button onClick={() => setIsHelpOpen(true)} className="p-2 rounded-full hover:bg-white/10"><HelpCircleIcon className="w-5 h-5" /></button>
